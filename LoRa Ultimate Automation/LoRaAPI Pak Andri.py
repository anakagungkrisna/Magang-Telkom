...
def cekDetailError(num, Sitename, tipe):
    k = 0
    cekPower = False
    cekEther = False
    cekMobile = False
    ...
    for x in range(Gateway):
        ...
        if(gatewayTrue == True): 
            ...
            while y < len((LoRa).get('gateways')[x].get('modules')): #ada 6 module, ada juga yg 5
                if( (str( (LoRa).get('gateways')[x].get('modules')[y].get('type') ) == "power" ) ) :
                    excelPower.append( str( (LoRa).get('gateways')[x].get('modules')[y].get('status')) )  
                    cekPower = True
                    got += 1
                if( (str( (LoRa).get('gateways')[x].get('modules')[y].get('type') ) == "eth" )):
                    excelEthernet.append( str( (LoRa).get('gateways')[x].get('modules')[y].get('status')) )
                    cekEther = True
                    got += 1
                if( (str( (LoRa).get('gateways')[x].get('modules')[y].get('type') ) == "wwan" )):
                    excelWWAN.append( str( (LoRa).get('gateways')[x].get('modules')[y].get('status')) )
                    cekMobile = True#print("mobile exist")
                    got += 1
                if( (str( (LoRa).get('gateways')[x].get('modules')[y].get('name') ) == "GNSS" )):
                    excelGNSS.append( str( (LoRa).get('gateways')[x].get('modules')[y].get('status')) )
                if( str( (LoRa).get('gateways')[x].get('modules')[y].get('type') ) == ""):
                    excelPower.append( str( (LoRa).get('gateways')[x].get('modules')[y].get('status')) )
                    cekPower = True
                    got += 1
                if(y == len((LoRa).get('gateways')[x].get('modules')) - 1):
                    #if( str( (LoRa).get('gateways')[x].get('modules')[y].get('type') ) == ""):
                        #excelPower.append( str( (LoRa).get('gateways')[x].get('modules')[y].get('status')) )
                    if(got == 0): 
                        cekPower = False
            ...
            excelLoRa.append( (LoRa).get('gateways')[x].get('lora').get('status') )
...
def buatTXTfile():
    ...
    for x in range(len(gatewayDown)):
        file.write("-" + gatewayDown[x] + "\n")
        
    file.write("\nNew Gateway Up: \n")
    x = 0
    for x in range(len(gatewayUp)):
        file.write("-" + gatewayUp[x] + "\n")
...
def buatFolder(file1, file2):
    ...
    saveDirectory = "D:/Krisna/Magang Telkom/Research/loraAPI/MAIN/" + datetime.datetime.now().strftime('%d-%m-%Y') #INI PERLU DISESUAIKAN DENGAN DIRECTORY PROGRAMNYA
...
Gateway = int((LoRa).get('total')) #seluruh Gateway yang didapat dari website everynet
#nonamePower = json.loads(LoRa)


ExcelFile = buatExcel(LoRa)

x = 0
for x in range(Gateway):
    arrGateway = (LoRa).get('gateways')[x].get('lora_networks')
    gatewayTrue = "4" in arrGateway
    #print(gatewayTrue)
    ...
    if(gatewayTrue == True): 
        Total += 1
        gatewayIndo.append(str((LoRa).get('gateways')[x].get('sitename')))
        """
        Pow = True
        ether = True
        mob = True
...
if os.path.isfile('dataset.xlsx'):
    ...
    for p in range(len(gatewayDown)):
        for q in range(Gateway):
            ...
            Hasil = cekDetailError(q, str(gatewayDown[p]) , "Down")
            if(Hasil == None): pass
            else: gatewayDown[p] = Hasil
    ...
    for p in range(len(gatewayUp)):
        for q in range(Gateway):
            Hasil = cekDetailError(q, str(gatewayUp[p]), "Up")
            if(Hasil == None): pass
    ...
    print("Total Power error : ", powerError) #Karna kebawa data yg gaada mobile
    print("Total Ethernet error: ", ethernetError)
    print("Total Mobile error: ", mobileError) #Karna ada 1 gateway yg memang gaada mobile nya. mac = b0fd0b7009650000. ini kehitungnya malah ke powerError 
    ...
    client.start()
    client.send_message(botUsername, 'Laporan pukul ' + laporanPukul + "\n\n" + "New Gateway Down: \n" + messageDown + "\n\n" + "New Gateway Up: \n" + messageUp )
    client.send_file(botUsername, 'D:/Krisna/Magang Telkom/Research/LoRaAPI/MAIN/' + TXTfile) #INI PERLU DISESUAIKAN DENGAN DIRECTORY PROGRAMNYA
    print("\nMessage successfully sent to telegram")
    buatFolder(ExcelFile, TXTfile)
